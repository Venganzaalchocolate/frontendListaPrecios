...
import { listaPrecios, obtenerCampaniasActivas, obtenerLogo } from "@/API/services";
...
export default function Home({ data }) {
  ...
  const [urllogo, seturlogo]=useState('')
  ...
  const addCampania = async (campania) => {
    ...
    seturlogo(await obtenerLogo(campania))
  }
  ...
  return (
    <>
      <Layout title={"Lista de precios"}>
        ...
        <MostrarPrecios logo={urllogo} campaniaSeleccionada={campaniaSeleccionada} viviendas={apartamento} data={data}/>
      </Layout>
    </>
  );
}
