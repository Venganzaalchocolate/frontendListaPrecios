...
export default function Listaprecios({ pis }) { // Define un componente funcional por defecto llamado Listaprecios que recibe un prop "pis" (un array de datos de viviendas).
  if (pis != null && pis != []) { // Comprueba si la lista de pisos no es nulo ni vacío.
    return ( // Si el array es válido, retorna el código JSX para renderizar la lista de precios.
      <div id={styles.contenedorlista}> 
        {pis.map((casa, index) => { // Recorre el array "pis" con map, asignando cada elemento a la variable "casa" e index al índice actual.
          const precioFormateado = casa.precioPropiedad.toLocaleString(); // Formatea el precio para separarlo con comas.
          return ( // Por cada vivienda, retorna una tarjeta individual.
            <div key={index} className={styles.tarjeta}>  {/* Crea un div con una clave única y la clase definida en el CSS */}
              <div className={styles.textoPrincipal}>  {/* Crea un div para el texto principal */}
                <label>VIVIENDA</label>  {/* Etiqueta para el título "VIVIENDA" */}
                <label>PRECIO</label>  {/* Etiqueta para el título "PRECIO" */}
                <p className={styles.nombre}>{casa.nombrePropiedad}</p>  {/* Muestra el nombre de la vivienda */}
                <p className={styles.precio}>{precioFormateado} €</p>  {/* Muestra el precio formateado */}
              </div>
              <div className={styles.datos}>  {/* Crea un div para los datos de la vivienda */}
